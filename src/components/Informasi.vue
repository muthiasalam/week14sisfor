<template>
    <div class="informasi bg-zinc-100 pt-2 pb-5">
    <div class="judul text-black text-[26px] ml-7 mr-8 mt-4 mb-4 font-semibold">
      INFORMASI
    </div>

    <div class="info-content flex-col items-center justify-center ml-8 mr-8">
      <div
        class="info-item1 bg-white w-128 h-24 mr-30 mb-5 drop-shadow hover:bg-slate-100"
      >
        <a href="#" class="flex">
          <div class="item1-top">
            <div class="item1-left flex">
              <div
                class="item1-date-container bg-red-700 flex flex-col justify-center items-center w-24 h-24 p-2"
              >
                <p class="pt-3 pr-3 pl-3 text-24 font-bold text-white">23</p>
                <p class="pb-3 pr-3 pl-3 text-11 text-white">SEP 2019</p>
              </div>
            </div>
          </div>
          <div class="item1-right items-center justify-items-center flex">
            <p class="text-black text-10 md:text-14 pt-3 pl-3 pb-3">
              THE 3RD EPI INTERNATIONAL CONFERENCE ON SCIENCE AND ENGINEERING
              2019
            </p>
          </div>
          <div
            class="panah flex items-center justify-center mr-10 ml-auto relative"
          >
            <img
              src="../assets/informasi/panah.png"
              class="md:w-15 md:h-12 w-8 h-6 inset-0"
            />
          </div>
        </a>
      </div>

      <div
        class="info-item2 bg-white w-128 h-24 mr-30 mb-5 drop-shadow hover:bg-slate-100"
      >
        <a href="#" class="flex">
          <div class="item2-top">
            <div class="item2-left flex">
              <div
                class="item1-date-container bg-red-700 flex flex-col justify-center items-center w-24 h-24 p-2"
              >
                <p class="pt-3 pr-3 pl-3 text-24 font-bold text-white">06</p>
                <p class="pb-3 pr-3 pl-3 text-11 text-white">AGU 2019</p>
              </div>
            </div>
          </div>
          <div class="item2-right items-center justify-items-center flex">
            <p class="text-black text-10 md:text-14 pt-3 pl-3 pb-3">
              WORKSHOP & SIMPOSIUM NASIONAL GEOMEKANIKA 5 2019
            </p>
          </div>
          <div
            class="panah flex items-center justify-center mr-10 ml-auto relative"
          >
            <img
              src="../assets/informasi/panah.png"
              class="md:w-15 md:h-12 w-8 h-6 inset-0"
            />
          </div>
        </a>
      </div>

      <div
        class="info-item3 bg-white w-128 h-24 mr-30 mb-5 drop-shadow hover:bg-slate-100"
      >
        <a href="#" class="flex">
          <div class="item3-top">
            <div class="item3-left flex">
              <div
                class="item3-date-container bg-red-700 flex flex-col justify-center items-center w-24 h-24 p-2"
              >
                <p class="pt-3 pr-3 pl-3 text-24 font-bold text-white">09</p>
                <p class="pb-3 pr-3 pl-3 text-11 text-white">JUL 2019</p>
              </div>
            </div>
          </div>
          <div class="item3-right items-center justify-items-center flex">
            <p class="text-black text-10 md:text-14 pt-3 pl-3 pb-3">
              PENGEMBANGAN KARAKTER MAHASISWA BARU 2019 FAKULTAS TEKNIK UNHAS
            </p>
          </div>
          <div
            class="panah flex items-center justify-center mr-10 ml-auto relative"
          >
            <img
              src="../assets/informasi/panah.png"
              class="md:w-15 md:h-12 w-8 h-6 inset-0"
            />
          </div>
        </a>
      </div>

      <div
        class="info-item4 bg-white w-128 h-24 mr-30 mb-5 drop-shadow hover:bg-slate-100"
      >
        <a href="#" class="flex">
          <div class="item4-top">
            <div class="item4-left flex">
              <div
                class="item4-date-container bg-red-700 flex flex-col justify-center items-center w-24 h-24 p-2"
              >
                <p class="pt-3 pr-3 pl-3 text-24 font-bold text-white">25</p>
                <p class="pb-3 pr-3 pl-3 text-11 text-white">JUN 2019</p>
              </div>
            </div>
          </div>
          <div class="item4-right items-center justify-items-center flex">
            <p class="text-black text-10 md:text-14 pt-3 pl-3 pb-3">
              INTERNATIONAL GUEST LECTURE ON JOURNAL ARTICLE
            </p>
          </div>
          <div
            class="panah flex items-center justify-center mr-10 ml-auto relative"
          >
            <img
              src="../assets/informasi/panah.png"
              class="md:w-15 md:h-12 w-8 h-6 inset-0"
            />
          </div>
        </a>
      </div>

      <a href="#" class="flex justify-end">
        <p
          class="text-gray-800 text-xs font-light inline-block w-auto h-auto ml-auto"
        >
          <span class="more-info text-xs font-light hover:text-blue-600"
            >LIHAT INFORMASI LAIN ></span
          >
        </p>
      </a>
    </div>
  </div>
</template>